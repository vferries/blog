...
- [ ] **`/about/`, `/blog/` et les 3 archives restent en `twitter:card: summary`** (petite vignette) : MM ne passe en `summary_large_image` que si `page_large_image` est défini, ce qui demande un `header.og_image` en front matter — `site.og_image` seul ne suffit pas. Décider si ces pages méritent une carte dédiée (titre propre, comme les billets) ou juste la carte générique en grand format. Recette pour cette seconde option : `header:` + `og_image: /images/og-card.png` dans le front matter de chaque page
...
- [ ] **Robustesse latente du générateur de vignettes** (relevé en revue, aucun cas réel aujourd'hui) : `tags()` ne gère que le style flow YAML (`tags: [a, b]`), pas le style bloc, et découpe naïvement sur la virgule ; `ratio()` sur un SVG dépend du délégué librsvg d'ImageMagick ; la branche « source plus haute que le ratio cible » de `derive_vignette()` n'est exercée par aucun bandeau actuel
