...
- [x] **Mouvement automatique > 5 s sans pause** (SC 2.2.2, niveau A) : vidéos réalisations en `loop` sans contrôle, marquee 40s, aurore CTA 12s, pastille pulse 2,4s partout → tranché le 2026-08-13 (voir arbitrages) : les vidéos perdent leur `loop` (`668d7b4`, le gros de l'écart), marquee/aurore/pastille assumés. La case était restée ouverte
...
- [x] **12 JPEG q90-100 oubliés par la passe du 2026-08-04** (mtime 23 avril) : 1 334 Ko → 632 Ko en q82, mêmes noms/URLs. Les pires : `rabot.jpg` q99, `startup-weekend.jpg` q99, `open_space.jpg` q98, `staples.jpg` q98. S → fait le 2026-08-13 (`058f84c`) : 11 fichiers, 1 222 → 560 Ko, et q85 sans 4:2:0 pour les deux logos (le chroma délavait le texte orange d'Agile Tour). La case était restée ouverte
...
- [x] **`profile_square.jpg` 800×800/69 Ko dans un avatar plafonné 110 px** sur chaque page MM. Attention : le même fichier sert `.ev-photo` (~640 px utiles) → deux fichiers. S → fait le 2026-08-13 (`4affdea`) : `profile_avatar.jpg` 220px/8,8 Ko pour la sidebar, `profile_square.jpg` reste la source de `.ev-photo`. Case restée ouverte
...
- [x] `images/peinard.jpg` (114 Ko) : plus aucune référence, toujours publié. Poids mort de dépôt. S → supprimé le 2026-08-13 dans `058f84c`, case restée ouverte
...
- [x] **Le seul lien billets → landing est en `rel="nofollow"`** (sidebar auteur, `_config.yml:70-72`) : 46 billets, 15 ans de backlinks, zéro signal transmis → réglé le 2026-08-13 par le pont commercial : chaque billet porte désormais deux liens follow en contenu (`/about/` et `/#services`). La sidebar reste nofollow (comportement MM), sans importance maintenant
...
- [x] **Pagination `/blog/page2-4/` au sitemap** avec title/description quasi identiques, sans noindex. S → noindex posé le 2026-08-14 (`4f2d0c0`) dans `head/custom.html`, gaté `paginator.page > 1` — vérifié au build : présent sur les pages 2-4, absent de `/blog/`. Elles restent au sitemap, assumé : paginate v1 ne différencie pas leur front matter, le noindex fait le travail
...
- [x] **Aucun pont commercial en fin de billet** : un billet se termine par tags → partage → Disqus → livré le 2026-08-13, copy validé : encart `ev-post-bridge` entre la date et le partage, liens follow vers `/about/` et `/#services`. Shadow (10ᵉ) de `_layouts/single.html` au delta d'une ligne (le contenu vit dans `_includes/ev-post-bridge.html`). Piège layout au passage : MM floate `.page__content`/`.page__meta`, sans `clear: both` l'aside mesurait 2155px avec son texte rendu 2000px plus bas. Vérifié au rendu deux thèmes
...
- [x] **LinkedIn absent du site** (`_data/social.yml` = GitHub/Twitch/X) alors que les billets proposent « Partager sur LinkedIn » — le réseau du prospect B2B → câblé le 2026-08-13 (URL fournie : linkedin.com/in/vferries) : `social.yml` (bloc contact landing + `/about/` + `sameAs` du JSON-LD automatiquement), sidebar auteur, icône SVG trait dans `ev-socials.html`
...
- [x] **Hero : aucun `error` handler** — vidéo manquante/404 = 190vh de scroll mort (`landing.js:147-152` engage le pin avant de savoir si ça charge) ; source mobile choisie une fois, jamais réévaluée en rotation ; couple 720px JS/CSS non documenté (contrairement au 1000/1001). S → fait le 2026-08-14 (`c0d4385`) : sur `error` le hero reprend sa mise en page no-JS (pin libéré, poster, accents allumés) — vérifié Playwright vidéo bloquée : hero d'un viewport, warn loggé. Couple 720 documenté dans le JS ; la source figée en rotation reste assumée (même séquence, seule la définition change)
- [x] **Handler scroll quick-nav non throttlé** : jusqu'à 7 `getBoundingClientRect()` + `scrollHeight` par événement (`landing.js:299`), alors que le pattern rAF existe 90 lignes plus haut. S → fait le 2026-08-14 (`4c10879`), même pattern rAF que le hero — vérifié Playwright, dots corrects en milieu et bas de page
...
- [x] **Code mort** : `.ev-container` défini, jamais utilisé, sa déclaration recopiée 13× ; `.ev-hero__scroll-hint` + 2 keyframes orphelins ; `.ev-clickable` dans le JS et le CSS, dans zéro template. S → retiré le 2026-08-14 (`e99de28`) : `.ev-container`, scroll-hint + `ev-drop`/`ev-fade-out`, `.ev-clickable` côtés CSS et JS
- [x] **Reset landing.css:39-51 cible des classes MM que la landing ne rend jamais** (`.masthead`, `.sidebar`, `.initial-content` — vérifié : 0 occurrence dans `_site/index.html`). Faux positif permanent de la procédure de bump. S → retiré le 2026-08-14 (`9d3c3c0`), après vérif que `#main` — seul sélecteur du bloc présent dans le markup landing — n'est contraint par aucune feuille chargée là (main.css du thème n'y arrive jamais)
...
- [x] **`_config.yml` : `jekyll-gist` sans aucun `{% gist %}`** et `twitter_username` à plat mort (MM lit la forme imbriquée) — illusion de source unique pour le pseudo X. S → purgés le 2026-08-14 (`ae61c70`), balises `twitter:*` revérifiées au build après retrait
...
- [ ] **Page « comment ça se passe »** (process premier échange → livraison) : l'objection n°1 d'un commerçant, absente du site → arbitré le 2026-08-13 : sera **dérivée du billet sites-vitrines** (même matière, une seule rédaction) une fois le brouillon rempli
